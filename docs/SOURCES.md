...
| Primary source | What was observed | What it does NOT prove |
|---|---|---|
| [Westwood One NFL schedule](https://www.westwoodonesports.com/nfl-schedule/) | All four page chunks re-read. **All 70 NFL rows in `nfl_westwoodone_2026.json` match the live page line by line** (matchup, date, ET air time, feed name, TBA slots, event IDs). No new or changed rows since the 2026-09-20 snapshot. | Local carriage per game; kickoff times (air time ≠ kickoff). |
